        ...
        Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final friendMatches = contactProvider.friends.where(
                (user) => user.id == story.authorId,
              );
              final friend = friendMatches.isEmpty ? null : friendMatches.first;
              final authorName = story.authorId == auth.user?.id
                  ? (auth.user?.displayName ?? story.authorId)
                  : (friend?.displayName ?? story.authorId);
              final authorAvatar = story.authorId == auth.user?.id
                  ? auth.user?.avatarUrl
                  : friend?.avatarUrl;
              return _buildStoryItem(isDarkMode, authorAvatar, authorName);
            },
          ),
        ),
        const SizedBox(height: 8),
