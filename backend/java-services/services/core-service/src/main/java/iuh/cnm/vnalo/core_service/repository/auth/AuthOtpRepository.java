...
public interface AuthOtpRepository extends JpaRepository<AuthOtp, UUID> {
     ...
     * Alias for findLatestValidOtp for backward compatibility.
     */
    ...
    Optional<AuthOtp> findLatestOtp(@Param("target") String target,
                                     @Param("purpose") OtpPurpose purpose);
     ...
     */
    @Query("SELECT COUNT(o) > 0 FROM AuthOtp o WHERE o.target = :target AND o.purpose = :purpose " +
           "AND o.verifiedAt IS NOT NULL")
    boolean existsVerifiedOtp(@Param("target") String target,
                               @Param("purpose") OtpPurpose purpose);

    /**
     * Count recent OTP requests for rate limiting (all purposes).
     */
    ...
    /**
     * Count recent OTP requests for rate limiting (specific purpose).
     */
    @Query("SELECT COUNT(o) FROM AuthOtp o WHERE o.target = :target AND o.purpose = :purpose " +
