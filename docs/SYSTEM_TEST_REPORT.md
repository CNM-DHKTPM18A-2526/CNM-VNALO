...
**Date:** 2026-03-14
**Environment:** Docker Compose (postgres, redis) + Local Spring Boot (media-service)
**Storage:** AWS S3 (`vnalo-media`)

---

## 12. Media Infrastructure Status

| Component | Technology | Port | Status | Health Check |
|-----------|-----------|------|--------|-------------|
| vnalo-postgres | PostgreSQL 16-alpine | 5432 | ✅ UP | `pg_isready` |
| media-service | Java 21 / Spring Boot 3.4.2 | 8083 | ✅ UP | `/actuator/health` |
| AWS S3 | Amazon Simple Storage Service | N/A | ✅ Connected | Object presign and put successful |
| message-service | Node.js / NestJS | 3000 | ✅ UP | Refactored recall + deleteForMe |

**Database:** 3 new tables added (`media_object`, `media_access_scope`, `sticker`, `sticker_pack` etc.)
**Schema:** Hibernate `ddl-auto: update`
**Auth:** JWT HS512 (Shared secret with core-service & message-service)

---

## 13. Bugs Found & Fixed in Media/Message Services

### Bug #4: Invalid `spring-dotenv` Version (CRITICAL)
- **Symptom:** Maven build failed to resolve `me.paulschwarz:spring-dotenv:4.0.0`
- **Root Cause:** Version 4.0.0 does not exist in Maven Central.
- **Fix:** Switched to Spring Boot's native environment variable handling and consolidated configuration into a single `application.yml` with default fallbacks for local development, removing the need for external dotenv libraries.

### Bug #5: Kafka Connection Warnings on Startup (MEDIUM)
- **Symptom:** `NetworkClient` spamming warnings: `Connection to node -1 (localhost:9092) could not be established`
- **Root Cause:** Kafka was defined in `docker-compose.yml` but not running locally while `media-service` expected a connection.
- **Fix:** Configured `kafka.admin.fail-fast: false`, `kafka.listener.auto-startup: false`, and suppressed logging for `org.apache.kafka.clients.NetworkClient` in `application.yml` via `root: WARN`.

### Bug #6: AWS S3 Hardcoded Secrets Leak Risk (CRITICAL)
- **Symptom:** AWS Credentials were raw strings in `application-dev.yml`
- **Root Cause:** Hardcoded strings in yaml files committed to GitHub expose keys.
- **Fix:** Removed dev/prod specific files. Centralized to `application.yml` using `${AWS_ACCESS_KEY_ID:}` injecting via OS Environment variables or IntelliJ Run Config.

### Feature/Bug #7: Missing "Delete for Me" in message-service (FEATURE)
- **Symptom:** Users could recall messages for everyone within 1 hour, but couldn't delete messages just from their own view indefinitely.
- **Fix:** Added `MessageUserDeletion` composite entity. Updated `getMessages()` query to filter out hidden messages. Added `DELETE /api/v1/messages/{id}/me` endpoint.

---

## 14. Media API Integration Test Coverage

### 14.1 Media Management API (14 Endpoints)

| # | Operation | Method | Endpoint | Status | Notes |
|---|-----------|--------|----------|--------|-------|
| 1 | Upload direct | POST | `/media/upload` | ✅ 201 | Multipart < 100MB |
| 2 | Initiate Upload | POST | `/media/initiate-upload` | ✅ 200 | Returns presigned URL |
| 3 | Complete Upload | POST | `/media/{id}/complete` | ✅ 200 | Updates status to READY |
| 4 | Get Info | GET | `/media/{id}` | ✅ 200 | Validates ownership/access |
| 5 | List Media | GET | `/media` | ✅ 200 | Paging + filtering |
| 6 | Delete Media | DELETE | `/media/{id}` | ✅ 200 | Deletes from DB & S3 |
| 7 | Update Meta | PATCH | `/media/{id}` | ✅ 200 | Filename/Category updates |
| 8 | Presign Download | GET | `/media/{id}/download` | ✅ 200 | 1 hr expiry |
| 9 | Save to Device | GET | `/media/{id}/save` | ✅ 200 | Streams bytes w/ `attachment` header |
| 10 | Get Thumbnail | GET | `/media/{id}/thumbnail` | ✅ 302/200 | Redirects to thumbnail |
| 11 | Get Status | GET | `/media/{id}/status` | ✅ 200 | |
| 12 | Grant Access | POST | `/media/{id}/access` | ✅ 201 | USER/CONVERSATION/PUBLIC |
| 13 | Revoke Access | DELETE | `/media/{id}/access` | ✅ 200 | |
| 14 | List Access | GET | `/media/{id}/access` | ✅ 200 | |

### 14.2 Sticker Management API (14 Endpoints)

| # | Operation | Method | Endpoint | Status | Notes |
|---|-----------|--------|----------|--------|-------|
| 15 | List Packs | GET | `/stickers/packs` | ✅ 200 | Filter by category |
| 16 | Pack Details | GET | `/stickers/packs/{id}` | ✅ 200 | Includes full sticker list |
| 17 | Create Pack | POST | `/stickers/packs` | ✅ 201 | Uploads thumbnail to S3 |
| 18 | Update Pack | PATCH | `/stickers/packs/{id}` | ✅ 200 | Metadata |
| 19 | Delete Pack | DELETE | `/stickers/packs/{id}` | ✅ 200 | Soft delete |
| 20 | Add Sticker | POST | `/stickers/packs/{id}/stickers` | ✅ 201 | Updates pack count |
| 21 | Get Sticker | GET | `/stickers/{id}` | ✅ 200 | |
| 22 | Delete Sticker | DELETE | `/stickers/{id}` | ✅ 200 | Decrements pack count |
| 23 | Search | GET | `/stickers/search` | ✅ 200 | By keyword or emoji |
| 24 | Download Pack | POST | `/stickers/packs/{id}/download`| ✅ 201 | Added to user's collection |
| 25 | Remove Pack | DELETE | `/stickers/packs/{id}/download`| ✅ 200 | Removed from collection |
| 26 | My Packs | GET | `/stickers/my-packs` | ✅ 200 | |
| 27 | Log Usage | POST | `/stickers/{id}/use` | ✅ 200 | Analytics/Recents |
| 28 | Recent Stickers | GET | `/stickers/recent` | ✅ 200 | Sorted by usage timestamp |

---

## 15. Cross-Service Functionality Verified

| Scenario | Component 1 | Component 2 | Status |
|----------|-------------|-------------|--------|
| **Authentication Sync** | `core-service` generates JWT | `media-service` validates JWT using same secret | ✅ Verified |
| **User Deletion Tracking** | Sender recalling message in `message-service` | Recipient client receiving sync | ✅ Verified |
| **Local Config Isolation** | Dev environment `.env` | Production `docker-compose` | ✅ Verified |
| **Graceful Degradation** | `media-service` startup | `kafka` cluster completely offline | ✅ Verified (won't crash) |

---

## 16. Database Schema Additions

### 16.1 Media Schema (PostgreSQL `vnalo_media`)
| Table | Purpose |
|-------|---------|
| `media_object` | Stores file metadata, AWS S3 keys, and size metrics. |
| `media_access_scope`| Maps media UUIDs to specific users or conversation UUIDs for RBAC. |
| `sticker_pack` | Manages author, category, and counters for grouped stickers. |
| `sticker` | Individual sticker entities mapped to packs. |
| `user_sticker_pack` | Tracks which users have downloaded/installed which packs. |
| `sticker_usage_log` | Tracks individual sticker usage for "Recent Stickers" functionality. |

### 16.2 Message Schema Updates (PostgreSQL `vnalo_core`)
| Table | Purpose |
|-------|---------|
| `message_user_deletion`| New table mapping `user_id` to `message_id` for "Delete for Me" soft-delete isolation. |

**Final Media-Service Readiness: FULLY DEPLOYABLE** ✅
